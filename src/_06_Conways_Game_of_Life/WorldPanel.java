...
public class WorldPanel extends JPanel implements MouseListener, ActionListener {
	...
	public WorldPanel(int w, int h, int cpr) {
		...
		for (int i = 0; i < cpr; i++) {
			for (int a = 0; a < cpr; a++) {
				cells[i][a] = new Cell(a * cellSize, i * cellSize, cellSize);
			}
		}
	}
	...
	public void randomizeCells() {
		...
		for (int i = 0; i < cellsPerRow; i++) {
			for (int a = 0; a < cellsPerRow; a++) {
				f = r.nextInt(2);
				if (f == 0) {
					cells[i][a].isAlive = true;
				} else {
	...
	public void step() {
		...
		for (int i = 0; i < cellsPerRow; i++) {
			for (int a = 0; a < cellsPerRow; a++) {
				livingNeighbors[i][a] = getLivingNeighbors(cells, i, a);
			}
		}
...
//		for (int i = 0; i < cellsPerRow; i++) {
//			for (int a = 0; a < cellsPerRow; a++) {
//				if (cells[i][a].getX() == e.getX()) {
//					if (cells[i][a].getY() == e.getY()) {
//						if (cells[i][a].isAlive == true) {
//							cells[i][a].isAlive = false;
//						} else {
//							cells[i][a].isAlive = true;
//						}
